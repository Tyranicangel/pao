<?php
	...
	$output=array();
	$query=pg_query("SELECT * FROM (SELECT * FROM npayments WHERE auduser='$user_id' AND billstatus='1' AND fyear='$finyear') AS a INNER JOIN (SELECT partyname,bankacno FROM pao_ddo_party) AS b ON a.bankcode=b.bankacno ORDER BY transid");
