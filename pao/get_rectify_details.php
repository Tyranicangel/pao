<?php
	...
	$tkno=$_POST['tkno'];
	$query=pg_query("SELECT * FROM npayments WHERE transid='$tkno'");
	$result=pg_fetch_array($query,null,PGSQL_ASSOC);
	$ft=$result['formtype'];
	$fno=$result['formno'];
	$hoa=$result['hoa'];
	...
	$q4=pg_fetch_array(pg_query("SELECT * FROM form_user_chk WHERE tokenno='$transid' AND userid='fin'"),null,PGSQL_ASSOC);
