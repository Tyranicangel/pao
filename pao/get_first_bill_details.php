<?php
	...
	$query=pg_query("SELECT * FROM npayments WHERE billstatus='4' AND fyear='$finyear' ORDER BY transid LIMIT 1");
	$result=pg_fetch_array($query,null,PGSQL_ASSOC);
	$ft=$result['formtype'];
	$fno=$result['formno'];
	...
	$q4=pg_fetch_array(pg_query("SELECT * FROM form_user_chk WHERE tokenno='$tkno' AND userid='off'"),null,PGSQL_ASSOC);
	$result['form_off_chk']=$q4;
