$(document).ready(function(){
		...
		success:function(result)
		{
			if(result.length>0)
			{
				for(var i=0;i<result.length;i++)
				{
					...
					$clone.html("<td>"+(i+1)+"</td><td>"+result[i]['transid']+"</td><td>"+getdate(result[i]['tokenissuedate'])+"</td><td>"+result[i]['partyname']+"</td><td>"+result[i]['ddodesg']+"</td><td>"+sort_hoa(result[i]['hoa'])+"</td><td>"+result[i]['gross']+"</td><td>"+result[i]['dedn']+"</td><td>"+result[i]['net']+"</td>");
	...
	$('#start_scrutiny').click(function(){
		...
		$.ajax({
			...
			success:function(result)
			{
				...
				$('.bill_det_wrap').show();
				if(result['form_aud_chk']['remarks']=='')
				{
					$('#aud_rems').html('None');	
				}
				else
				...
				if(result['form_sup_chk']['remarks']=='')
				{
					$('#sup_rems').html('None');	
				}
				else
				...
				}if(result['form_off_chk']['remarks']=='')
				{
					$('#off_rems').html('None');	
				}
				else
				{
					$('#off_rems').html(result['form_off_chk']['remarks']);
				}
				...
				var audchk=result['form_aud_chk']['form_data'].split('|');
				var audchkarr={};
				for(var i=0;i<audchk.length-1;i++)
				{
					var x=audchk[i].split(":");
					audchkarr[x[0]]=x[1];
				}
				var supchk=result['form_sup_chk']['form_data'].split('|');
				var supchkarr={};
				for(var i=0;i<supchk.length-1;i++)
				{
					var x=supchk[i].split(":");
					supchkarr[x[0]]=x[1];
				}
				var offchk=result['form_off_chk']['form_data'].split('|');
				var offchkarr={};
				for(var i=0;i<offchk.length-1;i++)
				{
					var x=offchk[i].split(":");
					offchkarr[x[0]]=x[1];
				}
				for(var i=0;i<result['frules'].length;i++)
				{
					...
					$clone.html("<td>"+(i+1)+"</td><td>"+result['frules'][i]['formdesc']+"</td><td>"+get_form_chk(ddochkarr[result['frules'][i]['formref']])+"</td><td>"+get_form_chk(audchkarr[result['frules'][i]['formref']])+"</td><td>"+get_form_chk(supchkarr[result['frules'][i]['formref']])+"</td><td>"+get_form_chk(offchkarr[result['frules'][i]['formref']])+"</td><td><input type='radio' name='auditor_radio"+(i+1)+"' class='aud_chk_yes'/></td><td><input type='radio' name='auditor_radio"+(i+1)+"' class='aud_chk_no'/></td><td><input type='radio' name='auditor_radio"+(i+1)+"' class='aud_chk_na'/></td>");
	...
	function update_button()
	{
		...
		if(billtype=='passing')
		{
			...
			{
				$('.pass_rej').html('Approve Bill').attr('id','bill_pass');
			}
		}
		else if(billtype=='frwd')
		{
			...
			{
				$('.pass_rej').html('Forward to Govt').attr('id','forward_to_supt');
			}
		}
	}
