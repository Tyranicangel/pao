<?php
	...
	$query=pg_fetch_all(pg_query("SELECT * FROM (SELECT transid,tokenissuedate,hoa,gross,bankcode FROM npayments WHERE billstatus = '23' AND blistuser='$user_id' AND passuser='$supt') AS a INNER JOIN (SELECT partyname,bankacno FROM pao_ddo_party) AS b ON a.bankcode = b.bankacno"));
