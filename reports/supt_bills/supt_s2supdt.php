<?php
	...
	$query=pg_fetch_all(pg_query("SELECT * FROM (SELECT transid,tokenissuedate,hoa,gross,bankcode FROM npayments WHERE billstatus = '22' AND passuser='$user_id' AND auduser='$auduser') AS a INNER JOIN (SELECT partyname,bankacno FROM pao_ddo_party) AS b ON a.bankcode = b.bankacno"));
