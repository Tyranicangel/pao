<?php
	...
	$query=pg_fetch_array(pg_query("SELECT password FROM users WHERE userid='$user_id'"),null,PGSQL_ASSOC);
	if($oldpass==$query['password'])
	{
		$query1=pg_query("UPDATE users SET password='$newpass' WHERE userid='$user_id'");
		echo 'success';
	}
	else
	{
